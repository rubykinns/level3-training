...
# =============================================================================
    ...
    # ── Show question ──────────────────────────────────────────────────────
        ...
        if st.button("Submit Answer →", type="primary", use_container_width=True):
            ...
            # Per-question feedback: concept layer + trust calibration layer
            if tg == "4":
                # Determine concept and trust layers based on group
                if kg in ("A", "B"):  # Low Knowledge
                    concept = q.get("feedback_low_k", q.get("explanation", ""))
                else:  # High Knowledge
                    concept = q.get("feedback_high_k", q.get("explanation", ""))
                if kg in ("A", "C"):  # Low Trust
                    trust = q.get("feedback_low_trust", "")
                else:  # High Trust
                    trust = q.get("feedback_high_trust", "")
                per_q_feedback = concept + (" " + trust if trust else "")
